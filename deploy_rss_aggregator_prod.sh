...
CHANNEL_TITLE="${RSS_AGG_CHANNEL_TITLE:-BioluxeLab_Aggregated_Feed}"
CHANNEL_LINK="${RSS_AGG_CHANNEL_LINK:-https://bioluxelab.com/}"
CHANNEL_DESCRIPTION="${RSS_AGG_CHANNEL_DESCRIPTION:-Aggregated_RSS_feeds_for_BioluxeLab}"
...
if [[ -f "$LOCAL_ENV_FILE" ]]; then
  [[ -n "$SHOPIFY_DOMAIN" ]] || SHOPIFY_DOMAIN="$(read_dotenv_value RSS_AGG_SHOPIFY_DOMAIN "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_DOMAIN" ]] || SHOPIFY_DOMAIN="$(read_dotenv_value MYSHOPIFY_DOMAIN "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_API_VERSION" ]] || SHOPIFY_API_VERSION="$(read_dotenv_value RSS_AGG_SHOPIFY_API_VERSION "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_API_VERSION" ]] || SHOPIFY_API_VERSION="$(read_dotenv_value SHOPIFY_API_VERSION "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_ADMIN_TOKEN" ]] || SHOPIFY_ADMIN_TOKEN="$(read_dotenv_value RSS_AGG_SHOPIFY_ADMIN_TOKEN "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_ADMIN_TOKEN" ]] || SHOPIFY_ADMIN_TOKEN="$(read_dotenv_value SHOPIFY_ADMIN_ACCESS_TOKEN "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_CLIENT_ID" ]] || SHOPIFY_CLIENT_ID="$(read_dotenv_value RSS_AGG_SHOPIFY_CLIENT_ID "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_CLIENT_ID" ]] || SHOPIFY_CLIENT_ID="$(read_dotenv_value SHOPIFY_CLIENT_ID "$LOCAL_ENV_FILE" || true)"
  [[ -n "$SHOPIFY_CLIENT_SECRET" ]] || SHOPIFY_CLIENT_SECRET="$(read_dotenv_value RSS_AGG_SHOPIFY_CLIENT_SECRET "$LOCAL_ENV_FILE" || true)"
...
ExecStart=/usr/bin/python3 $REMOTE_APP_DIR/rss_aggregator.py --feeds-file $REMOTE_APP_DIR/rss_feeds.json --host 127.0.0.1 --port $RSS_PORT --cache-ttl 300 --timeout 20 --max-items $RSS_MAX_ITEMS --channel-link $CHANNEL_LINK --channel-title "$CHANNEL_TITLE" --channel-description "$CHANNEL_DESCRIPTION"
Restart=always
RestartSec=5
